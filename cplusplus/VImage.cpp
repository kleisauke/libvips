...
std::vector<double>
negate(const std::vector<double> &vector)
{
...
std::vector<double>
invert(const std::vector<double> &vector)
{
	std::vector<double> new_vector(vector.size());
...
// input int array
VOption *
VOption::set(const char *name, const std::vector<int> &value)
{
	Pair *pair = new Pair(name);
...
// input double array
VOption *
VOption::set(const char *name, const std::vector<double> &value)
{
	Pair *pair = new Pair(name);
...
// input image array
VOption *
VOption::set(const char *name, const std::vector<VImage> &value)
{
	Pair *pair = new Pair(name);
...
VImage
VImage::new_from_source(const VSource &source, const char *option_string,
	VOption *options)
{
	const char *operation_name;
	VImage out;
...
void
VImage::write_to_target(const char *suffix, const VTarget &target,
	VOption *options) const
{
	char filename[VIPS_PATH_MAX];
	char option_string[VIPS_PATH_MAX];
...
VImage
VImage::bandjoin(const VImage &other, VOption *options) const
{
...
VImage
VImage::composite(const VImage &other, VipsBlendMode mode, VOption *options) const
{
	VImage v[2] = { *this, other };
