...
vips_foreign_load_fits_dispose(GObject *gobject)
{
	...
	G_OBJECT_CLASS(vips_foreign_load_fits_parent_class)->dispose(gobject);
}
...
vips_foreign_load_fits_file_class_init(VipsForeignLoadFitsFileClass *class)
{
	...
	VIPS_ARG_STRING(class, "filename", 1,
		...
		VIPS_STRUCT_OFFSET(VipsForeignLoadFitsFile, filename),
		NULL);
}
	...
	VipsForeignLoadFitsSourceClass *class)
{
	...
	VIPS_ARG_OBJECT(class, "source", 1,
		...
		VIPS_ARGUMENT_REQUIRED_INPUT,
		VIPS_STRUCT_OFFSET(VipsForeignLoadFitsSource, source),
		VIPS_TYPE_SOURCE);
}
