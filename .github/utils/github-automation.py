...
def setup_bot_git(git_dir="."):
    """
    Configure the git repo in `git_dir` with the GitHub actions account.
    """
    repo = Repo(git_dir)
    with repo.config_writer() as config:
        # https://github.com/actions/checkout?tab=readme-ov-file#push-a-commit-using-the-built-in-token
        config.set_value("user", "name", "github-actions[bot]")
        config.set_value("user", "email", "41898282+github-actions[bot]@users.noreply.github.com")
...
class BackportWorkflow:
    ...
    def create_pull_request(
        ...
        is 8.15, then the base branch will be 8.15. `branch` will be used as
        the compare branch.
...
backport_workflow_parser.add_argument(
    "--branch-repo",
    type=str,
    default="libvips/libvips",
    help="The name of the repo where new branches will be pushed (e.g. libvips/libvips)",
)
backport_workflow_parser.add_argument(
    "sub_command",
    type=str,
...
bot_git_config_parser = subparsers.add_parser(
    "setup-bot-git",
    help="Set the default user and email for the git repo in LIBVIPS_PROJECT_DIR to github-actions[bot]",
)
backport_workflow_parser.add_argument(
    "--requested-by",
    type=str,
...
if args.command == "backport-workflow":
    ...
    if args.sub_command == "print-release-branch":
        backport_workflow.print_release_branch()
    else:
        if not backport_workflow.execute_command():
            sys.exit(1)
elif args.command == "setup-bot-git":
    setup_bot_git()
